...
#include <iostream>
#include <fstream>
std::string RESOURCE_PATH = "/Users/mac/OpenCV/MyOpenCVAI/MyOpenCVAI/resources";
std::string OUTPUT_PATH = "/Users/mac/OpenCV/MyOpenCVAI/MyOpenCVAI/output";
...
void ImageToGCode(Mat &srcImg, int linePerMillimetre, int engraveWidth, int engraveHeight, int sMax, int speed, bool isLineToLine, bool compressed, string savePath)
{
    Mat M1 = srcImg;
    Mat M2;
...
int main() { 
	...
	cv::cvtColor(srcImg, srcImg, CV_BGRA2GRAY);
	cv::resize(srcImg, srcImg, cv::Size(), 0.5, 0.5);
	cv::imshow("srcImg", srcImg);
	ImageToGCode(srcImg, 12, srcImg.cols / 10, srcImg.rows / 10, 1000, 2500, false, true, OUTPUT_PATH + "/huahui.gcode");
	cv::waitKey(0);
